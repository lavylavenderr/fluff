...
module.exports = {
    command: "kiss",
    description:
        "Want to kiss someone or yourself? This is the command to do so!",
    data: new SlashCommandBuilder()
        ...
        .addUserOption((option) =>
            option
                .setName("user")
                .setDescription("The user you'd like to kiss!")
        )
    ...
    run: async (client, interaction) => {
        ...
        if (User.id === interaction.member.id) {
            interaction.reply(
                `<@${User.id}> kisses the floor.. a bit awkward don't ya think?`
            );
        } else {
            interaction.reply(
                `${interaction.member.user.username} ${smooch[random]}`
            );
        }
    },
};
