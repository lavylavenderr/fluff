...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option =>
            option.setName('source')
                .setDescription('The site the GIF will be grabbed from.')
                .setRequired(true)
                .addChoices(
                    {
                        name: 'Giphy',
    ...
    run: async (client, interaction) => {
        ...
        try {
            if (source == "giphy") {
                const request = await (await axios.get(`https://api.giphy.com/v1/gifs/search?api_key=${process.env.GIPHYAPIKEY}&q=${query}&limit=1&offset=0&rating=g&lang=en&format=json`)).data
